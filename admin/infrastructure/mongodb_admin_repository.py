...
from admin.domain.admin_model import Admin, admin_schema, admins_schema
...
class MongoDBSuperAdminRepository(ISuperAdminRepository):
    ...
    def get_all(self) -> list[Admin]:
        ...
        if results:
            return [Admin(**admin) for admin in admins_schema(results)]
        return []
    
    def update(self, admin_to_update: Admin) -> bool:
        admin_dict = vars(admin_to_update)
        admin_dict.pop("admin_id")
        update_dict = {"$set": admin_dict}
        result = self.__collection.update_one(
            {"email": admin_to_update.email}, update_dict)
